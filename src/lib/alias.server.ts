...
export async function provisionAliasForUser(userId: string): Promise<AliasResult> {
  ...
  const { data: profile } = await supabaseAdmin
    .from("profiles")
    .select(
      "username, forwarding_email, forwarding_email_verified, is_paid, verified, status, is_banned, is_suspended",
    )
    .eq("id", userId)
    .maybeSingle();
  ...
  if (!entitled) {
    await supabaseAdmin.from("profiles").update({ alias_status: "none" }).eq("id", userId);
  ...
  if (forward && profile.forwarding_email_verified !== true) {
    ...
    await supabaseAdmin.from("profiles").update({ alias_status: "pending" }).eq("id", userId);
